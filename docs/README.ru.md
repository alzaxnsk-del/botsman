...
> Это операционная документация на русском. Интерфейс бота, установщик и все сообщения системы — на английском (проект международный). Основной README — [в корне репозитория](../README.md).
...
1. **одной строкой** ставится на твой VPS (`curl … | bash`);
2. **одним сообщением** из описания рождается *живой* сервис — код, база, поддомен, HTTPS, скриншот — по реальному адресу;
3. **одной командой** подключается к Claude Code на ноутбуке; `git push` передеплоит.

Как чат-в-приложение у Replit, но на **своём** сервере и со **своим** ключом — без наценки на токены и без вендор-лока. Или как локальный Claude Code, только без боли с продакшеном — домен, TLS, контейнер и база уже подключены.

```
Ты:      «сделай TODO-сервис со списком задач и возможностью отмечать выполненные»
Botsman: ✓ todo-spisok-zadach.example.com — задеплоен
         [ссылка] [скриншот]
         Что поправить?
...
- Домен с **wildcard DNS-записью**: у DNS-провайдера создай A-запись с хостом `*` и значением = публичный IP сервера — тогда любой поддомен (`todo.example.com`, …) ведёт на сервер. Проверка: `dig +short anything.example.com` → IP сервера. (Если домен общий с сайтом — возьми суб-базу вроде `apps.example.com` с хостом `*.apps`.) Без неё ссылки и TLS не заработают; бот проверяет запись и предупредит. В Cloudflare запись должна быть **DNS only** (серое облако): прокси ломает выпуск Let's Encrypt-сертификатов.
...
| `/memory <slug>` | что агент помнит о проекте (его `CLAUDE.md`) |
...
| `/setup` | смена авторизации агента, **модели**, домена или телеметрии — прямо в чате |
| `/version` | текущая версия и дата релиза |
| `/server` `/projects` `/home` | переключение «комнат» (см. ниже) |

### Как общаться с ботом

Режимов запоминать не нужно — просто пиши, что хочешь, бот маршрутизирует по смыслу:
- **Создать** — «сделай TODO-сервис» → новый сервис задеплоен.
- **Изменить** — «добавь тёмную тему» → правит текущий проект и передеплоит.
- **Спросить** — «как это устроено?», «что в логах?» → ответ без деплоя.
- **Эксплуатация сервера** — «посмотри нагрузку», «почисти диск», «перезапусти todo», «обнови сервер» → операции. Чтение — сразу; изменения — по кнопке, операции уровня хоста (обновление ОС, самообновление) — с двойным подтверждением.

Постоянные кнопки под полем меняются в зависимости от контекста и работают **без обращения к LLM** — то есть переживают сбой или исчерпанный лимит ключа:

- **🏠 Home** — панель управления: статус (живые проекты, упавшие контейнеры, доступность LLM-роутера, предупреждения старта) и кнопки **📊 Server status · 📦 Projects · ⬆️ Update Botsman · 🔧 Setup · 💻 Code on your computer**. Чтобы создать проект — просто опиши его прямо здесь.
- **📦 Подключение к проекту** (кнопкой в **📦 Projects** или «go to todo») — пока подключён, все правки и вопросы идут в него, а на клавиатуре — действия проекта: **🔍 Review** (read-only обзор кода), **📋 Logs**, **↩️ Rollback** (с подтверждением), **💻 Claude Code** (готовая команда clone + `claude` + `git push`). Выход — **🚪 Exit**.
- **🛠 Server** — админ-режим: спрашивай про сервер обычным языком, частые действия продублированы кнопками. Чтение — сразу; изменения — с подтверждением, операции уровня хоста — с двойным.

Маршрутизация вне подключения — по смыслу; неоднозначное бот переспрашивает кнопкой. Клавиатуры — дополнительная надёжная поверхность, а не «режим»: одна и та же фраза значит одно и то же где угодно.
...
Подключись к проекту и нажми **💻 Claude Code** (или `/status <slug>`) — бот покажет готовую команду клонирования, подставив публичный IP сервера, путь к репозиторию и SSH-пользователя (выводится из пути установки). Дальше:

```bash
git clone <user>@<server-ip>:~/.botsman/repos/<slug>.git
cd <slug> && claude       # дорабатываешь в Claude Code
...
| `baseDomain` | задаётся в чате | базовый домен, например `example.com`; запрашивается ботом при онбординге |
...
| `agent.model` | нет | модель Claude: `opus` / `sonnet` / `haiku` (по умолчанию `opus`); меняется в чате через `/setup` без перезапуска |
...
- Очередь задач строго последовательная: вторая просьба ждёт первую. Смена модели применяется без перезапуска; если демон всё же перезапускается по другой причине, прерванное создание проекта можно возобновить одной кнопкой (исходный запрос сохраняется), а прерванные правки помечаются failed и повторяются вручную.
