...
Постоянные кнопки под полем — необязательные ярлыки: **🏠** сбрасывает фокус, **🛠 Server** показывает, что можно спросить про сервер, **📦 Projects** фокусирует проект (чтобы «сделай крупнее» относилось к нему). Маршрутизация никогда не зависит от «режима» — одна и та же фраза значит одно и то же где угодно.
