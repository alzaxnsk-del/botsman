...
Постоянные кнопки под полем ввода переключают три режима общения:
- **🏠 Home** — создание и правки сервисов (как раньше).
- **🛠 Server** — DevOps-ассистент, которому пишешь человеческим языком: «посмотри нагрузку», «почисти диск», «перезапусти todo», «обнови сервер». Чтение — сразу; изменения — по кнопке подтверждения, операции уровня хоста (обновление ОС, самообновление Botsman) — с двойным подтверждением.
- **📦 Projects** — заходишь в проект и продолжаешь разработку в чате: сообщение трактуется как **правка** (→ деплой) или **вопрос** (→ ответ без деплоя).
