...
- Домен с **wildcard DNS-записью**: у DNS-провайдера создай A-запись с хостом `*.apps` (или `*`, если весь домен под Botsman) и значением = публичный IP сервера — тогда любой поддомен (`todo.apps.example.com`, …) ведёт на сервер. Проверка: `dig +short anything.apps.example.com` → IP сервера. Без неё ссылки и TLS не заработают; мастер настройки проверяет запись и предупредит. В Cloudflare запись должна быть **DNS only** (серое облако): прокси ломает выпуск Let's Encrypt-сертификатов (5xx или петля редиректов).
